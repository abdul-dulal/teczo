<script setup>
import logo from "@/assets/img/logo/logo.png";
import virtual from "@/assets/img/home/virtual.png";
import wave from "@/assets/img/home/wave02.png";
import { ArrowRight } from "@lucide/vue";
const currentYear = new Date().getFullYear();
</script>

<template>
  <div class="flex flex-wrap mt-[120px] items-center justify-end">
    <button
      class="xl:px-[102px] px-10 py-[34px] bg-purple paragraph text-white cursor-pointer"
    >
      Saturday - Thursday : 8:30 am - 10:30 pm
    </button>

    <button
      class="xl:px-[60px] px-10 xl:w-[746px] w-[395px] cursor-pointer flex justify-between py-[34px] bg-bg paragraph text-white"
    >
      Call for Appoinment <arrow-right />
    </button>
  </div>
  <div class="bg-bg section-base relative">
    <div class="absolute xl:top-2 -bottom-[86px] right-0">
      <img :src="virtual" alt="Virtual" />
    </div>
    <div class="absolute top-0 left-0">
      <img :src="wave" alt="Wave" />
    </div>
    <div class="container">
      <div
        class="grid lg:grid-cols-4 sm:grid-cols-2 xl:gap-[140px] gap-10 items-start justify-between"
      >
        <div class="space-y-7">
          <img :src="logo" alt="Logo" />
          <p class="paragraph text-white/75 max-w-[273px]">
            Desire to obtain pain of itself because is is pain, but occasionally
            circumstances
          </p>
          <div class="flex gap-3 items-center ml-4 xl:pr-0 pr-5">
            <a
              href="https://www.facebook.com/"
              target="_blank"
              rel="noopener noreferrer "
              class="shadow-[0px_1px_2px_0px_#01103D24] size-10 bg-white/5 rounded-[10px] flex items-center justify-center group transition-all ease-in-out duration-500 z-9999 hover:bg-white"
            >
              <svg
                width="8"
                height="14"
                viewBox="0 0 8 14"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                class="group-hover:text-black cursor-pointer text-white"
              >
                <path
                  d="M1.88265 13.9168H4.70664V6.95842H7.26587L7.53062 4.56646H4.70664V3.37048C4.70664 3.00807 4.76547 2.75437 4.88313 2.60941C5.0008 2.46444 5.28516 2.39196 5.73621 2.39196H7.53062V0H4.94197C3.76531 0 2.96126 0.262753 2.52982 0.788258C2.09838 1.29564 1.88265 2.0839 1.88265 3.15303V4.56646H0V6.95842H1.88265V13.9168Z"
                  fill="currentColor"
                />
              </svg>
            </a>
            <a
              href="https://x.com/"
              target="_blank"
              rel="noopener noreferrer"
              class="shadow-[0px_1px_2px_0px_#01103D24] size-10 bg-white/5 hover:bg-white rounded-[10px] flex items-center justify-center group cursor-pointer z-9999 transition-all ease-in-out duration-500"
            >
              <svg
                width="14"
                height="12"
                viewBox="0 0 14 12"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                class="group-hover:text-black cursor-pointer text-white"
              >
                <path
                  d="M13.603 0.21745C13.0385 0.543626 12.4285 0.770137 11.7729 0.896983C11.2084 0.298994 10.5073 0 9.66961 0C8.88658 0 8.2128 0.280874 7.64828 0.842621C7.08377 1.40437 6.80151 2.07484 6.80151 2.85404C6.80151 3.07149 6.82882 3.28894 6.88345 3.50639C5.718 3.45203 4.61629 3.16209 3.5783 2.63659C2.55853 2.11108 1.69355 1.40437 0.983351 0.516445C0.710198 0.951346 0.573621 1.43155 0.573621 1.95705C0.573621 2.97182 1.00156 3.76008 1.85744 4.32183C1.40219 4.32183 0.974246 4.20404 0.573621 3.96847V4.02283C0.573621 4.69331 0.783038 5.2913 1.20187 5.8168C1.63892 6.34231 2.19433 6.67754 2.86811 6.82251C2.61316 6.87687 2.35822 6.90405 2.10328 6.90405C1.92118 6.90405 1.73907 6.88593 1.55697 6.84969C1.75728 7.42956 2.09417 7.90976 2.56764 8.2903C3.05931 8.65272 3.61472 8.83393 4.23387 8.83393C3.19589 9.64937 2.01223 10.0571 0.682882 10.0571C0.46436 10.0571 0.236733 10.048 0 10.0299C1.34755 10.8816 2.81348 11.3074 4.39776 11.3074C5.67248 11.3074 6.83793 11.0628 7.89412 10.5735C8.95031 10.0843 9.80619 9.45004 10.4618 8.67084C11.1355 7.87352 11.6545 7.00372 12.0187 6.06143C12.3829 5.10103 12.565 4.14062 12.565 3.18021C12.565 3.05337 12.565 2.93558 12.565 2.82686C13.1296 2.41008 13.603 1.91175 13.9854 1.33188C13.4573 1.56746 12.911 1.72148 12.3465 1.79397C12.9657 1.41343 13.3845 0.887923 13.603 0.21745Z"
                  fill="currentColor"
                />
              </svg>
            </a>

            <a
              href="https://www.linkedin.com/"
              target="_blank"
              rel="noopener noreferrer"
              class="shadow-[0px_1px_2px_0px_#01103D24] size-10 bg-white/5 hover:bg-white z-9999 rounded-[10px] flex items-center justify-center group transition-all ease-in-out duration-500"
              :class="className"
            >
              <svg
                width="13"
                height="13"
                viewBox="0 0 13 13"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                class="group-hover:text-black cursor-pointer text-white"
              >
                <path
                  d="M0.0308789 3.37631H2.96437V12.6612H0.0308789V3.37631ZM8.98575 3.37631C8.84165 3.37631 8.69755 3.3851 8.55344 3.40269C8.42993 3.42027 8.30641 3.44665 8.1829 3.48182C8.05938 3.51699 7.95645 3.55216 7.87411 3.58733C7.79177 3.60491 7.68884 3.64887 7.56532 3.71921C7.44181 3.77197 7.35946 3.80714 7.31829 3.82472C7.2977 3.84231 7.22565 3.89506 7.10214 3.98299C6.97862 4.07091 6.91686 4.11488 6.91686 4.11488V3.37631H3.98337V12.6612H6.91686V6.80537C6.91686 6.78779 6.91686 6.75262 6.91686 6.69986C6.91686 6.64711 6.93745 6.55039 6.97862 6.40971C7.04038 6.26903 7.11243 6.15473 7.19477 6.0668C7.2977 5.96129 7.4521 5.86458 7.65796 5.77665C7.8844 5.68873 8.14173 5.64477 8.42993 5.64477C8.92399 5.64477 9.28424 5.75027 9.51069 5.96129C9.75772 6.17231 9.88124 6.45367 9.88124 6.80537V12.6612H13V6.91088C13 6.29541 12.8868 5.76786 12.6603 5.32824C12.4339 4.87103 12.1663 4.52812 11.8575 4.29952C11.5693 4.05333 11.2193 3.85989 10.8076 3.71921C10.4165 3.56095 10.0768 3.46423 9.7886 3.42906C9.52098 3.39389 9.25337 3.37631 8.98575 3.37631ZM0.432304 0.369284C0.144101 0.615473 0 0.914417 0 1.26612C0 1.61781 0.144101 1.91676 0.432304 2.16295C0.720507 2.40914 1.07047 2.53223 1.48219 2.53223C1.8939 2.53223 2.24386 2.40914 2.53207 2.16295C2.82027 1.91676 2.96437 1.61781 2.96437 1.26612C2.96437 0.914417 2.82027 0.615473 2.53207 0.369284C2.24386 0.123095 1.8939 0 1.48219 0C1.07047 0 0.720507 0.123095 0.432304 0.369284Z"
                  fill="currentColor"
                />
              </svg>
            </a>
            <a
              href="https://www.instagram.com/"
              target="_blank"
              rel="noopener noreferrer"
              class="shadow-[0px_1px_2px_0px_#01103D24] size-10 bg-white/5 hover:bg-white z-9999 rounded-[10px] flex items-center justify-center group transition-all ease-in-out duration-500"
              :class="className"
            >
              <svg
                width="15"
                height="15"
                viewBox="0 0 15 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                class="group-hover:text-black cursor-pointer text-white"
              >
                <path
                  d="M7.50615 1.35085C9.51171 1.35085 9.7492 1.35964 10.5379 1.3948C11.271 1.42704 11.6668 1.55011 11.9307 1.65267C12.2796 1.78746 12.5318 1.95155 12.7927 2.21235C13.0566 2.47607 13.2179 2.72514 13.3527 3.07384C13.4554 3.33757 13.5785 3.73608 13.6108 4.46572C13.646 5.25689 13.6548 5.49424 13.6548 7.49561C13.6548 9.4999 13.646 9.73726 13.6108 10.5255C13.5785 11.2581 13.4554 11.6536 13.3527 11.9174C13.2179 12.2661 13.0537 12.5181 12.7927 12.7789C12.5288 13.0426 12.2796 13.2038 11.9307 13.3385C11.6668 13.4411 11.268 13.5642 10.5379 13.5964C9.74627 13.6316 9.50877 13.6404 7.50615 13.6404C5.5006 13.6404 5.2631 13.6316 4.47437 13.5964C3.74135 13.5642 3.34552 13.4411 3.08163 13.3385C2.73271 13.2038 2.48055 13.0397 2.21959 12.7789C1.95571 12.5151 1.79444 12.2661 1.65956 11.9174C1.55694 11.6536 1.43379 11.2551 1.40154 10.5255C1.36635 9.73432 1.35756 9.49697 1.35756 7.49561C1.35756 5.49131 1.36635 5.25396 1.40154 4.46572C1.43379 3.73315 1.55694 3.33757 1.65956 3.07384C1.79444 2.72514 1.95864 2.47314 2.21959 2.21235C2.48348 1.94862 2.73271 1.78746 3.08163 1.65267C3.34552 1.55011 3.74428 1.42704 4.47437 1.3948C5.2631 1.35964 5.5006 1.35085 7.50615 1.35085ZM7.50615 0C5.46835 0 5.21326 0.00879078 4.4128 0.0439539C3.61527 0.079117 3.06697 0.208048 2.59197 0.392655C2.09645 0.586052 1.67716 0.840985 1.2608 1.26001C0.84151 1.67611 0.586418 2.09514 0.3929 2.58742C0.208178 3.06505 0.0791665 3.61008 0.0439814 4.40711C0.00879628 5.21 0 5.46493 0 7.50147C0 9.538 0.00879628 9.79293 0.0439814 10.5929C0.0791665 11.3899 0.208178 11.9379 0.3929 12.4126C0.586418 12.9078 0.84151 13.3268 1.2608 13.7429C1.67716 14.159 2.09645 14.4169 2.58904 14.6073C3.06697 14.792 3.61234 14.9209 4.40987 14.956C5.21033 14.9912 5.46542 15 7.50322 15C9.54103 15 9.79612 14.9912 10.5966 14.956C11.3941 14.9209 11.9424 14.792 12.4174 14.6073C12.91 14.4169 13.3293 14.159 13.7456 13.7429C14.162 13.3268 14.42 12.9078 14.6106 12.4155C14.7953 11.9379 14.9243 11.3929 14.9595 10.5958C14.9947 9.79586 15.0035 9.54093 15.0035 7.5044C15.0035 5.46787 14.9947 5.21293 14.9595 4.41297C14.9243 3.61594 14.7953 3.06798 14.6106 2.59328C14.4259 2.09514 14.1708 1.67611 13.7515 1.26001C13.3352 0.843915 12.9159 0.586052 12.4233 0.395585C11.9453 0.210979 11.4 0.0820473 10.6024 0.0468842C9.79905 0.00879078 9.54396 0 7.50615 0Z"
                  fill="currentColor"
                />
                <path
                  d="M7.5056 3.64844C5.3769 3.64844 3.6499 5.37436 3.6499 7.50173C3.6499 9.6291 5.3769 11.355 7.5056 11.355C9.6343 11.355 11.3613 9.6291 11.3613 7.50173C11.3613 5.37436 9.6343 3.64844 7.5056 3.64844ZM7.5056 10.0012C6.12459 10.0012 5.00453 8.88188 5.00453 7.50173C5.00453 6.12158 6.12459 5.00222 7.5056 5.00222C8.88662 5.00222 10.0067 6.12158 10.0067 7.50173C10.0067 8.88188 8.88662 10.0012 7.5056 10.0012Z"
                  fill="currentColor"
                />
                <path
                  d="M12.4141 3.49529C12.4141 3.99344 12.0094 4.39489 11.5139 4.39489C11.0155 4.39489 10.6138 3.99051 10.6138 3.49529C10.6138 2.99715 11.0184 2.5957 11.5139 2.5957C12.0094 2.5957 12.4141 3.00008 12.4141 3.49529Z"
                  fill="currentColor"
                />
              </svg>
            </a>
          </div>
        </div>
        <div class="space-y-4">
          <h4 class="heading-four text-white">Explore</h4>
          <ul class="paragraph text-white font-medium space-y-3 lg:ml-0 ml-5">
            <li class="list-disc">
              <router-link to="/about">About Us</router-link>
            </li>
            <li class="list-disc">
              <router-link to="/team"> Meet our team </router-link>
            </li>
            <li class="list-disc">
              <router-link to="/blog"> News & media </router-link>
            </li>
            <li class="list-disc">
              <router-link to="/portfolio"> Our projects </router-link>
            </li>
            <li class="list-disc">
              <router-link to="/contact"> Contacts</router-link>
            </li>
          </ul>
        </div>
        <div class="space-y-4">
          <h4 class="heading-four text-white">Service link</h4>
          <ul class="paragraph text-white font-medium space-y-3 lg:ml-0 ml-5">
            <li class="list-disc">
              <router-link to="/services">Data visualization </router-link>
            </li>
            <li class="list-disc">
              <router-link to="/services"> UI/UX Designing </router-link>
            </li>
            <li class="list-disc">
              <router-link to="/services"> Digital marketing </router-link>
            </li>
            <li class="list-disc">
              <router-link to="/services">Data analytics </router-link>
            </li>
            <li class="list-disc">
              <router-link to="/services"> Security system</router-link>
            </li>
          </ul>
        </div>
        <div class="space-y-4 relative z-9999">
          <h4 class="heading-four text-white">Contact</h4>
          <p class="paragraph">
            401 Broadway, 24th Floor, Orchard View, London
          </p>
          <div class="space-y-6">
            <a
              href="mailto:teczoinfo@gmail.com"
              class="flex items-center gap-4 text-white"
            >
              <svg
                width="18"
                height="16"
                viewBox="0 0 18 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M0.75 4.75C0.75 1.95 2.35 0.75 4.75 0.75H12.75C15.15 0.75 16.75 1.95 16.75 4.75V10.35C16.75 13.15 15.15 14.35 12.75 14.35H4.75"
                  stroke="#4BF0C5"
                  stroke-width="1.5"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                />
                <path
                  d="M12.75 5.15039L10.246 7.15015C9.422 7.80607 8.07 7.80607 7.246 7.15015L4.75 5.15039"
                  stroke="#4BF0C5"
                  stroke-width="1.5"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                />
                <path
                  d="M0.75 11.1504H5.55"
                  stroke="#4BF0C5"
                  stroke-width="1.5"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                />
                <path
                  d="M0.75 7.94922H3.15"
                  stroke="#4BF0C5"
                  stroke-width="1.5"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                />
              </svg>

              <span>teczoinfo@gmail.com</span>
            </a>
            <a
              href="https://wa.me/3331231688"
              target="_blank"
              class="flex items-center gap-4 text-white"
            >
              <svg
                width="18"
                height="18"
                viewBox="0 0 18 18"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M9.6377 0.75C9.6377 0.75 11.593 0.927758 14.0816 3.41637C16.5703 5.90498 16.748 7.86032 16.748 7.86032"
                  stroke="#4BF0C5"
                  stroke-width="1.5"
                  stroke-linecap="round"
                />
                <path
                  d="M9.82227 3.8916C9.82227 3.8916 10.7021 4.14299 12.0219 5.46278C13.3417 6.78256 13.5931 7.66242 13.5931 7.66242"
                  stroke="#4BF0C5"
                  stroke-width="1.5"
                  stroke-linecap="round"
                />
                <path
                  d="M6.11615 3.6991L6.69299 4.7327C7.21355 5.66547 7.00458 6.8891 6.18469 7.70899C6.18469 7.70899 5.1902 8.7035 6.99332 10.5066C8.79641 12.3097 9.79095 11.3152 9.79095 11.3152C10.6108 10.4954 11.8345 10.2864 12.7672 10.807L13.8008 11.3838C15.2093 12.1698 15.3757 14.1451 14.1376 15.3832C13.3937 16.1271 12.4824 16.7059 11.475 16.7441C9.77902 16.8084 6.8989 16.3792 4.00982 13.4901C1.12074 10.601 0.691531 7.72092 0.755823 6.02499C0.794015 5.01755 1.37287 4.10622 2.11679 3.3623C3.35483 2.12426 5.33009 2.2906 6.11615 3.6991Z"
                  stroke="#4BF0C5"
                  stroke-width="1.5"
                  stroke-linecap="round"
                />
              </svg>

              <span>(+333) 123-1688</span>
            </a>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="bg-bg">
    <div class="text-center border-t border-white/10 py-[30px]">
      <p class="paragraph text-white/75 font-inter mb-0 font-medium">
        Copyright &copy; {{ currentYear }}
        <a href="/" class="text-secondary">Teczo</a>. All Rights Reserved.
      </p>
    </div>
  </div>
</template>
